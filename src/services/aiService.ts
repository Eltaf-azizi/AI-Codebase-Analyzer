...
export class AIService {
  ...
  static async initialize(files: FileData[]) {
    ...
    try {
      ...
      throw new Error(`Failed to initialize search: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  static async analyzeProject(files: FileData[]): Promise<AnalysisResult> {
    try {
      ...
      try {
        ...
        // Return a default analysis result if JSON parsing fails
        return {
          summary: "Analysis completed but formatting was incomplete. Check console for details.",
          architecture: "Architecture visualization unavailable",
          features: ["Unable to extract features"],
          improvements: ["Unable to extract improvements"],
          security: ["Unable to analyze security"],
          architectureData: { nodes: [], links: [] }
        };
      }
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : 'Unknown error';
      throw new Error(`Project analysis failed: ${errorMsg}`);
    }
  }
  ...
  static async chat(files: FileData[], message: string, history: ChatMessage[]): Promise<string> {
    try {
      ...
      let searchResults = [];
      try {
        searchResults = await vectorStore.search(message, 10);
      } catch (searchError) {
        console.warn('Semantic search failed, falling back to keyword search:', searchError);
      }
      
      const context = searchResults
        .map(res => `File: ${res.metadata.path}\nLines: ${res.metadata.startLine}-${res.metadata.endLine}\nContent:\n${res.metadata.content || res.id}`)
        .join('\n\n---\n\n');

      // Fallback to keyword search if semantic search is too sparse
